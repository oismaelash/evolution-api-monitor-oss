...
import { ossAccessSetupSchema } from '@monitor/shared';
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = ossAccessSetupSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
    }
    const token = await AccessLockService.setupPassword(parsed.data.password);
