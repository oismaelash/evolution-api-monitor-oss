...
export function HealthChart24h({ buckets }: { buckets: Bucket[] }) {
  ...
  return (
    <div className="group/chart relative mb-10 overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-lg transition-all duration-300 hover:shadow-2xl hover:shadow-accent/5">
        ...
        {buckets.map((row, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Tooltip */}
              <div className="pointer-events-none absolute bottom-full left-1/2 mb-4 w-36 -translate-x-1/2 scale-90 translate-y-2 opacity-0 transition-all duration-200 group-hover:scale-100 group-hover:translate-y-0 group-hover:opacity-100 z-20">
                <div className="rounded-xl bg-[#1e293b]/95 p-3 text-xs text-white backdrop-blur-xl shadow-2xl border border-white/10 ring-1 ring-black/20">
                  <div className="mb-2 border-b border-white/10 pb-1 font-bold text-gray-300 flex justify-between">
                    <span>{label}</span>
                    <span className="text-[10px] uppercase tracking-wider">{t('Checks', 'Checks')}</span>
                  </div>
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center bg-white/5 rounded px-1.5 py-0.5">
                ...
                {/* Arrow */}
                <div className="mx-auto -mt-1 h-2 w-2 rotate-45 border-r border-b border-white/10 bg-[#1e293b]/95" />
              </div>
            </div>
          );
        })}
