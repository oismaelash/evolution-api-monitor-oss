---
name: monitor-status-api
description: >-
  Integra alertas WhatsApp (API pilotstatus.online): x-api-key, TEST vs LIVE, POST /v1/messages/send
...
# Alertas WhatsApp — integração no Evolution API Monitor (API + produto externo)
...
## Opt-in em LIVE (número do provedor)

- Destinatário deve ter consentimento explícito para receber no número do provedor em LIVE.
- Opt-in é registrado quando o usuário envia mensagem ao WhatsApp do provedor (não só via API).
...
1. Chamar a API só de **worker** ou **servidor** — nunca do browser.
2. Encapsular em **AlertProvider**; env (`MONITOR_STATUS_API_KEY` ou nome validado no projeto).
...
const res = await fetch('https://pilotstatus.online/v1/messages/send', {
  ...
  headers: {
    ...
    'x-api-key': process.env.MONITOR_STATUS_API_KEY!,
  },
  body: JSON.stringify({
    templateId,
    destinationNumber,
...
- Rule: `.cursor/rules/010-monitor-status-api.mdc`
